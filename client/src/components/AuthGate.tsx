/**
 * SIGES iPhone Native: acceso agrupado, directo y táctil; la invitación mantiene
...
function SetupPending() {
  ...
  return <main className="min-h-screen bg-background text-foreground"><div className="grid min-h-screen lg:grid-cols-[17.5rem_minmax(0,1fr)]"><aside className="hidden border-r bg-card p-5 lg:flex lg:flex-col"><div className="flex items-center gap-3 px-2"><BrandMark /><div><b className="text-lg tracking-tight">Sistema <span className="text-[#0F8F73]">SIGES</span></b><p className="mt-0.5 text-[10px] font-bold uppercase tracking-[.13em] text-muted-foreground">Operaciones</p></div></div><nav className="mt-10 space-y-1">{items.map(({ label, icon: Icon }, index) => <div className={`side-link ${index === 0 ? "active" : "opacity-50"}`} key={label}><Icon size={18} />{label}<LockKeyhole className="ml-auto" size={13} /></div>)}</nav><div className="mt-auto rounded-lg border border-[#0F8F73]/15 bg-[#0F8F73]/5 p-3"><p className="text-xs font-extrabold text-[#08745D] dark:text-[#5DDBC0]">ESTADO DEL ESPACIO</p><p className="mt-2 font-mono text-sm font-bold">0 de 4 conexiones activas</p><p className="mt-1 text-xs leading-5 text-muted-foreground">La agenda y los permisos se activarán al conectar Firebase.</p></div></aside><section className="min-w-0 p-5 sm:p-8 lg:p-10"><header className="mb-10 flex items-center justify-between border-b pb-5"><div className="flex items-center gap-2 lg:hidden"><BrandMark /><b>Sistema SIGES</b></div><span className="connection-status-icon-only is-offline ml-auto" role="status" aria-live="polite" title="Sin conexión a internet; los cambios quedarán pendientes de sincronización"><span aria-hidden="true"><WifiOff size={17} strokeWidth={2.25} /></span><span className="sr-only">Sin conexión a internet; los cambios quedarán pendientes de sincronización</span></span></header><div className="max-w-5xl"><p className="eyebrow">Sistema · Preparación inicial</p><h1 className="mt-2 max-w-2xl text-4xl font-extrabold tracking-[-.065em] sm:text-5xl">Configuración pendiente: conecta Firebase para activar agenda, pagos y equipo.</h1><p className="mt-5 max-w-2xl text-base leading-7 text-muted-foreground">Sistema SIGES está listo para operar, pero todavía no tiene una fuente de identidad ni una base de datos vinculadas. Completa las variables de Firebase y publica las reglas de acceso.</p><section className="mt-9 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{checks.map(([label, detail, status], index) => <article className="metric-card !min-h-0" key={label}><div><p className="font-mono text-[10px] font-extrabold uppercase tracking-[.12em] text-muted-foreground">{label}</p><p className="mt-3 font-extrabold">{detail}</p><span className={`mt-3 inline-flex rounded-md px-2 py-1 font-mono text-[10px] font-extrabold uppercase tracking-[.08em] ${index < 2 ? "bg-amber-400/15 text-amber-700 dark:text-amber-300" : "bg-muted text-muted-foreground"}`}>{status}</span></div><span className={`h-9 w-1 ${index < 2 ? "bg-amber-400" : "bg-muted"}`} /></article>)}</section><section className="mt-7 grid gap-6 xl:grid-cols-[1.15fr_.85fr]"><article className="panel-card overflow-hidden"><div className="border-b px-5 py-4"><p className="font-extrabold">Ruta de activación</p><p className="mt-0.5 text-xs text-muted-foreground">Tres acciones para poner el sistema en marcha</p></div><ol className="divide-y">{[["01", "Registra la aplicación web", "Copia los seis valores de configuración de Firebase a tus variables VITE_FIREBASE_* ."], ["02", "Habilita identidad y base de datos", "Activa Email/Password, crea Cloud Firestore y publica el archivo firestore.rules."], ["03", "Crea el primer administrador", "Añade el perfil de administrador con su UID en la colección users; la guía explica el proceso." ]].map(([number, title, text]) => <li className="flex gap-4 px-5 py-5" key={number}><span className="time-code text-sm font-semibold text-[#0F8F73]">{number}</span><div><p className="font-extrabold">{title}</p><p className="mt-1 text-sm leading-6 text-muted-foreground">{text}</p></div></li>)}</ol></article><article className="relative overflow-hidden rounded-xl bg-[#102B35] p-6 text-white"><div className="absolute inset-0 auth-pattern opacity-40" /><div className="relative"><div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#0F8F73] text-white"><Database size={20} /></div><p className="mt-12 font-mono text-xs font-extrabold uppercase tracking-[.14em] text-[#9FE9D6]">Documentación incluida</p><h2 className="mt-2 text-2xl font-extrabold tracking-tight">Tu operación comienza con una base segura.</h2><p className="mt-4 max-w-md text-sm leading-6 text-white/75">Abre <code className="rounded bg-white/10 px-1.5 py-0.5 text-[#B5EFE0]">README.md</code> para seguir el procedimiento exacto de Firebase y GitHub Pages.</p></div></article></section></div></section></div></main>;
}
        ...
        <aside className="auth-brand-panel" aria-label="Identidad SIGES">
          ...
          <div className="auth-brand-inner">
            ...
            <div className="auth-brand-center">
              ...
              <p className="auth-brand-caption">Sistema Integral de Gestión Estratégica</p>
              <span className="auth-brand-subtitle">Acceso seguro</span>
            </div>
          </div>
        </aside>
