...
type Section = "overview" | "calendar" | "mail" | "reservations" | "customers" | "payments" | "products" | "tasks" | "hr" | "updates" | "automations" | "hr_reports" | "performance" | "impact" | "finance" | "employees" | "history" | "operations" | "access" | "pending" | "reminders" | "profile";
const sections: Section[] = ["overview", "calendar", "mail", "reservations", "customers", "payments", "products", "tasks", "hr", "updates", "automations", "hr_reports", "performance", "impact", "finance", "employees", "history", "operations", "access", "pending", "reminders", "profile"];
...
function AppSidebar({ active, onNavigate, isAdmin, isIT, collapsed, onToggle }: { active: Section; onNavigate: (section: Section) => void; isAdmin: boolean; isIT: boolean; collapsed: boolean; onToggle: () => void }) {
  ...
  if (isAdmin) groups.push({ id: "admin", label: "Administración", icon: ShieldAlert, items: [{ key: "pending", label: "Pendientes", icon: CheckCircle2 }, { key: "operations", label: "Operación", icon: ClipboardList }, { key: "automations", label: "Automatizaciones", icon: Bot }, { key: "history", label: "Historial", icon: History }] });
