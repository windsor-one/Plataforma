...
import {
  ...
  recordCarbonUsage, subscribeCarbonUsage, subscribeSecuritySettings, updateEmployee, updateOwnProfile, updateRecord, updateSecuritySettings,
} from "@/lib/firestore";
import type { AccessLog, ActivityLog, CarbonUsage, Customer, Expense, GeneralReminder, Incident, Invitation, Payment, PaymentKind, PaymentMethod, PaymentStatus, Product, ProductCategory, Reservation, ReservationStatus, SecuritySettings, Task, UserProfile, UserRole } from "@/lib/types";
...
type Section = "overview" | "reservations" | "customers" | "payments" | "products" | "tasks" | "impact" | "finance" | "employees" | "history" | "operations" | "access" | "reminders" | "profile";
...
function AppSidebar({ active, onNavigate, isAdmin, onLogout, collapsed, onToggle }: { active: Section; onNavigate: (section: Section) => void; isAdmin: boolean; onLogout: () => void; collapsed: boolean; onToggle: () => void }) {
  const base: Array<{ key: Section; label: string; icon: LucideIcon }> = [
    ...
    { key: "customers", label: "Clientes", icon: UsersRound }, { key: "payments", label: "Pagos", icon: CreditCard }, { key: "products", label: "Productos", icon: CircleDollarSign }, { key: "tasks", label: "Tareas", icon: ClipboardList }, { key: "impact", label: "Impacto digital", icon: Leaf }, { key: "reminders", label: "Notificaciones", icon: Bell },
  ];
