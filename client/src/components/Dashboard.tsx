...
type Section = "overview" | "reservations" | "customers" | "payments" | "products" | "tasks" | "hr" | "hr_reports" | "performance" | "impact" | "finance" | "employees" | "history" | "operations" | "access" | "pending" | "reminders" | "profile";
...
function AppSidebar({ active, onNavigate, isAdmin, collapsed, onToggle }: { active: Section; onNavigate: (section: Section) => void; isAdmin: boolean; collapsed: boolean; onToggle: () => void }) {
  ...
  if (isAdmin) groups.push({ id: "admin", label: "Administración", icon: ShieldAlert, items: [{ key: "pending", label: "Pendientes", icon: CheckCircle2 }, { key: "employees", label: "Personal", icon: UserCog }, { key: "operations", label: "Operación", icon: ClipboardList }, { key: "history", label: "Historial", icon: History }, { key: "access", label: "Accesos y actividad", icon: ShieldAlert }] });
...
export default function Dashboard({ user, profile }: { user: User; profile: UserProfile }) {
  ...
  const profilePanel = section === "pending" && isAdmin ? pendingPanel : <section className="max-w-2xl">
