...
import {
  ...
  recordCarbonUsage, subscribeCarbonUsage, subscribeEmployeeHrRecords, subscribeHrPolicies, subscribeOwnHrProfile, subscribeSecuritySettings, updateEmployee, updateOwnProfile, updateRecord, updateSecuritySettings,
} from "@/lib/firestore";
import type { AccessLog, ActivityLog, AttendanceRecord, CarbonUsage, Customer, EmploymentContract, Expense, GeneralReminder, HrDocument, HrGoal, HrPolicy, HrProfile, Incident, Invitation, LeaveRequest, LifecycleChecklist, OrganizationUnit, Payment, PaymentKind, PaymentMethod, PaymentStatus, PerformanceReview, PolicyAcknowledgment, Product, ProductCategory, Recognition, Reservation, ReservationStatus, SecuritySettings, Task, TrainingRecord, UserProfile, UserRole, WorkSchedule } from "@/lib/types";
...
export default function Dashboard({ user, profile }: { user: User; profile: UserProfile }) {
  ...
  useEffect(() => {
    ...
    void recordCarbonUsage(user.uid, profile, transferredBytes, resources.length + (navigation ? 1 : 0)).then(() => window.sessionStorage.setItem(sessionKey, "recorded")).catch(() => window.sessionStorage.removeItem(sessionKey));
  }, [profile, user.uid]);
