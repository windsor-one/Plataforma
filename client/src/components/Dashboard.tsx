...
export default function Dashboard({ user, profile }: { user: User; profile: UserProfile }) {
  ...
  const modalTitle = modal?.type === "customer" ? `${modal.data ? "Editar" : "Nuevo"} cliente` : modal?.type === "reservation" ? `${modal.data ? "Editar" : "Nueva"} reserva` : modal?.type === "payment" ? `${modal.data ? "Editar" : "Registrar"} pago` : modal?.type === "reminder" ? "General reminders" : modal?.data ? "Configurar empleado" : "Invitar empleado";
