...
import {
  createGeneralReminder, createRecord, deleteEmployeeProfile, deleteGeneralReminder, inviteEmployee, removeRecord, subscribeCollection,
  updateEmployee, updateOwnProfile, updateRecord,
} from "@/lib/firestore";
import type { ActivityLog, Customer, GeneralReminder, Payment, PaymentMethod, PaymentStatus, Reservation, ReservationStatus, UserProfile, UserRole } from "@/lib/types";
...
function GeneralReminderForm({ userId, onDone }: { userId: string; onDone: () => void }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await createGeneralReminder({ title: String(form.get("title")).trim(), message: String(form.get("message")).trim(), priority: String(form.get("priority")) as GeneralReminder["priority"] }, userId);
      event.currentTarget.reset();
      toast.success("Aviso publicado para el equipo.");
      onDone();
    } catch { toast.error("No fue posible publicar el aviso. Comprueba las reglas de Firestore."); } finally { setSubmitting(false); }
  };
  return <form className="form-stack" onSubmit={submit}><label>Asunto<input className="field" name="title" required placeholder="Ej. Reunión general" /></label><label>Mensaje<textarea className="field min-h-28" name="message" required placeholder="Escribe el aviso que verá el equipo…" /></label><label>Prioridad<select className="field" name="priority" defaultValue="info"><option value="info">Informativo</option><option value="important">Importante</option><option value="urgent">Urgente</option></select></label><button className="primary-button" disabled={submitting}>{submitting ? "Publicando…" : "Publicar aviso"}<ChevronRight size={16} /></button></form>;
}
...
function AppSidebar({ active, onNavigate, isAdmin, onLogout, collapsed, onToggle }: { active: Section; onNavigate: (section: Section) => void; isAdmin: boolean; onLogout: () => void; collapsed: boolean; onToggle: () => void }) {
  ...
  if (isAdmin) base.push({ key: "employees", label: "Empleados", icon: UserCog }, { key: "reminders", label: "General reminders", icon: Bell }, { key: "history", label: "Historial", icon: History });
...
export default function Dashboard({ user, profile }: { user: User; profile: UserProfile }) {
  ...
  const modalTitle = modal?.detail?.title || (modal?.type === "customer" ? `${modal.data ? "Editar" : "Nuevo"} cliente` : modal?.type === "reservation" ? `${modal.data ? "Editar" : "Nueva"} reserva` : modal?.type === "payment" ? `${modal.data ? "Editar" : "Registrar"} pago` : modal?.type === "reminder" ? "General reminders" : modal?.data ? "Configurar empleado" : "Invitar empleado");
