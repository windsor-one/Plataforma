...
import {
  ...
  Eye, History, LayoutDashboard, LockKeyhole, LogOut, Menu, Moon, Pencil, Plus, Search, ShieldAlert, Sun, Trash2,
...
import {
  bulkDeleteEmployeeProfiles, bulkRemoveRecords, bulkUpdateEmployees, bulkUpdateRecords, createGeneralReminder, createRecord, deleteActivityLog, deleteEmployeeProfile, deleteGeneralReminder, deleteProduct, inviteEmployee, removeRecord, saveProduct, subscribeCollection, updateGeneralReminder,
...
import type { AccessLog, ActivityLog, AttendanceRecord, AttendanceSettings, CarbonUsage, Customer, EmploymentContract, Expense, GeneralReminder, HrDocument, HrGoal, HrPolicy, HrProfile, Incident, Invitation, LeaveRequest, LifecycleChecklist, OrganizationUnit, Payment, PaymentKind, PaymentMethod, PaymentStatus, PerformanceReview, PolicyAcknowledgment, Product, ProductCategory, Recognition, Reservation, ReservationStatus, SecuritySettings, Task, TrainingRecord, UserProfile, UserRole, WorkSchedule } from "@/lib/types";
...
type Section = "overview" | "reservations" | "customers" | "payments" | "products" | "tasks" | "hr" | "hr_reports" | "performance" | "impact" | "finance" | "employees" | "history" | "operations" | "access" | "reminders" | "profile";
...
function AccountMenu({ profile, onNavigate, onLogout }: { profile: UserProfile; onNavigate: (section: Section) => void; onLogout: () => void }) {
  ...
  return <DropdownMenu open={open} onOpenChange={setOpen}><DropdownMenuTrigger asChild><button aria-label={`Abrir menú de ${profile.displayName}`} onPointerEnter={() => setOpen(true)} className="flex items-center gap-2 rounded-xl border bg-card px-2 py-1.5 text-left transition-transform duration-150 hover:-translate-y-0.5 hover:bg-muted/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0F8F73]" type="button"><span className="grid h-8 w-8 place-items-center rounded-lg bg-[#0F8F73] text-xs font-extrabold text-white">{initials}</span><span className="hidden max-w-32 truncate pr-1 text-xs font-bold sm:block">{profile.displayName}</span></button></DropdownMenuTrigger><DropdownMenuContent align="end" className="floating-surface w-56 rounded-xl border-border/80 p-1.5" onPointerEnter={() => setOpen(true)} onPointerLeave={() => setOpen(false)}><DropdownMenuLabel className="px-2.5 py-2"><p className="truncate text-sm font-extrabold">{profile.displayName}</p><p className="mt-0.5 truncate text-xs font-normal text-muted-foreground">{profile.email}</p></DropdownMenuLabel><DropdownMenuSeparator /><DropdownMenuItem className="cursor-pointer rounded-lg px-2.5 py-2" onSelect={() => onNavigate("profile")}><UserRound size={16} />Perfil</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem variant="destructive" className="cursor-pointer rounded-lg px-2.5 py-2" onSelect={onLogout}><LogOut size={16} />Cerrar sesión</DropdownMenuItem></DropdownMenuContent></DropdownMenu>;
}

function AppSidebar({ active, onNavigate, isAdmin, collapsed, onToggle }: { active: Section; onNavigate: (section: Section) => void; isAdmin: boolean; collapsed: boolean; onToggle: () => void }) {
  const base: Array<{ key: Section; label: string; icon: LucideIcon }> = [
    ...
    { key: "customers", label: "Clientes", icon: UsersRound }, { key: "payments", label: "Pagos", icon: CreditCard }, { key: "products", label: "Productos", icon: CircleDollarSign }, { key: "tasks", label: "Tareas", icon: ClipboardList }, { key: "hr", label: "Recursos Humanos", icon: UserCog }, { key: "impact", label: "Impacto digital", icon: Leaf }, { key: "reminders", label: "Notificaciones", icon: Bell },
  ];
  if (isAdmin) base.push({ key: "hr_reports", label: "Reportes RR. HH.", icon: FileDown }, { key: "performance", label: "Rendimiento", icon: BarChart3 }, { key: "finance", label: "Finanzas", icon: BarChart3 }, { key: "employees", label: "Personal", icon: UserCog }, { key: "operations", label: "Operación", icon: ClipboardList }, { key: "history", label: "Historial", icon: History }, { key: "access", label: "Accesos", icon: ShieldAlert });
  return <aside className={`sidebar-panel ${collapsed ? "is-collapsed" : ""}`}><div className="flex items-center justify-between gap-2 px-2"><button onClick={() => onNavigate("overview")} title="Ir al Resumen" className={`flex min-w-0 items-center gap-3 text-left ${collapsed ? "justify-center" : ""}`}><img src="/manus-storage/gestionpro-mark_51952039.png" alt="Sistema Heliot" className="h-11 w-11 shrink-0 rounded-xl bg-white p-1" />{!collapsed && <div><span className="text-lg font-extrabold tracking-tight">Sistema <span className="text-[#0F8F73]">Heliot</span></span><p className="mt-0.5 text-[10px] font-bold uppercase tracking-[.13em] text-muted-foreground">Operaciones</p></div>}</button><button className="icon-button" onClick={onToggle} title={collapsed ? "Expandir menú" : "Colapsar menú"}><Menu size={18} /></button></div><nav className="mt-10 space-y-1">{base.map(({ key, label, icon: Icon }) => <button key={key} title={collapsed ? label : undefined} onClick={() => onNavigate(key)} className={`side-link ${active === key ? "active" : ""} ${collapsed ? "justify-center px-2" : ""}`}><Icon size={18} />{!collapsed && label}</button>)}</nav><div className="mt-auto border-t pt-5">{!collapsed && <p className="px-3 pt-3 text-[10px] font-semibold uppercase tracking-[.12em] text-muted-foreground">Con tecnología de Windsor</p>}</div></aside>;
}
...
export default function Dashboard({ user, profile }: { user: User; profile: UserProfile }) {
  ...
  useEffect(() => {
    const sessionKey = `heliot-carbon-recorded-${user.uid}`;
    ...
    const updateSession = () => { if (!impactSessionId.current) return; captureActive(); const organization = hrProfile?.departmentId ? { departmentId: hrProfile.departmentId, departmentName: hrProfile.department || "Sin departamento" } : {}; void updateCarbonUsageSession(impactSessionId.current, user.uid, { activeMilliseconds: Math.round(activeMilliseconds), operationCount: impactInteractions.current, pageViews: impactPageViews.current, ...organization }); };
    ...
    const interval = window.setInterval(updateSession, 20_000);
    ...
    const impactContext = hrProfile?.departmentId ? { departmentId: hrProfile.departmentId, departmentName: hrProfile.department || "Sin departamento", deviceClass } : { deviceClass };
    ...
    return () => { window.clearInterval(interval); document.removeEventListener("visibilitychange", changeVisibility); window.removeEventListener("pointerdown", countInteraction); window.removeEventListener("keydown", countInteraction); updateSession(); };
  }, [hrProfile?.department, hrProfile?.departmentId, profile, user.uid]);
  ...
  const navigate = (next: Section) => { impactPageViews.current += 1; impactInteractions.current += 1; setSection(next); setMobileOpen(false); setQueryText(""); };
  ...
  const historyPanel = <><PageTitle eyebrow="Auditoría administrativa" title="Historial de movimientos" /><p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">Selecciona un movimiento para ver responsable, fecha, código y los datos del registro relacionado. Administración/IT puede retirar movimientos del historial; Personal no puede acceder a este panel.</p><section className="panel-card mt-6 overflow-hidden">{visibleHistory.length ? <div className="table-wrap"><table><thead><tr><th>Movimiento</th><th>Responsable</th><th>Fecha y hora</th><th aria-label="Eliminar" /></tr></thead><tbody>{visibleHistory.map((activity) => <tr className="cursor-pointer" onClick={() => openDetail(activity.entity === "reservation" ? "reservation" : activity.entity === "payment" ? "payment" : activity.entity === "product" ? "product" : activity.entity === "reminder" ? "reminder" : "customer", "Movimiento de historial", activity.summary, activity, linkedActivityRecord(activity))} key={activity.id}><td><p className="font-bold">{activity.summary}</p><span className="capitalize">{activity.entity}</span></td><td><p className="font-semibold">{activity.actorName}</p><span>{activity.actorEmail || "Sin correo"}</span></td><td>{readableTimestamp(activity.occurredAt)}</td><td><button className="icon-button danger" title="Eliminar movimiento" onClick={async (event) => { event.stopPropagation(); if (!window.confirm("¿Eliminar este movimiento del historial?")) return; try { await deleteActivityLog(activity.id); toast.success("Movimiento eliminado del historial."); } catch { toast.error("No se pudo eliminar el movimiento."); } }}><Trash2 size={16} /></button></td></tr>)}</tbody></table></div> : <Empty title="Aún no hay movimientos" detail="Los nuevos registros y cambios aparecerán aquí automáticamente." />}</section></>;
