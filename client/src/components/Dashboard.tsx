...
import {
  ...
  Eye, History, LayoutDashboard, LogOut, Menu, Moon, Pencil, Plus, Search, ShieldAlert, Sun, Trash2,
...
import type { AccessLog, ActivityLog, Customer, GeneralReminder, Invitation, Payment, PaymentKind, PaymentMethod, PaymentStatus, Product, ProductCategory, Reservation, ReservationStatus, UserProfile, UserRole } from "@/lib/types";
import { resolveProducts } from "@/lib/products";
import { downloadPaymentInvoice } from "@/lib/invoice";
import { useTheme } from "@/contexts/ThemeContext";

type Section = "overview" | "reservations" | "customers" | "payments" | "products" | "employees" | "history" | "operations" | "access" | "reminders" | "profile";
type RecordType = "customer" | "reservation" | "payment" | "product" | "employee" | "reminder";
type RecordData = Customer | Reservation | Payment | Product | UserProfile | GeneralReminder;
...
function RecordActions({ onView, onEdit, onDelete, onInvoice }: { onView?: () => void; onEdit: () => void; onDelete: () => void; onInvoice?: () => void }) {
  return <div className="flex justify-end gap-1">{onView && <button className="icon-button" title="Ver detalles" onClick={(event) => { event.stopPropagation(); onView(); }}><Eye size={16} /></button>}{onInvoice && <button className="icon-button" title="Descargar comprobante PDF" onClick={(event) => { event.stopPropagation(); onInvoice(); }}><FileDown size={16} /></button>}<button className="icon-button" title="Editar" onClick={(event) => { event.stopPropagation(); onEdit(); }}><Pencil size={16} /></button><button className="icon-button danger" title="Eliminar" onClick={(event) => { event.stopPropagation(); onDelete(); }}><Trash2 size={16} /></button></div>;
}
...
function AppSidebar({ active, onNavigate, isAdmin, onLogout, collapsed, onToggle }: { active: Section; onNavigate: (section: Section) => void; isAdmin: boolean; onLogout: () => void; collapsed: boolean; onToggle: () => void }) {
  const base: Array<{ key: Section; label: string; icon: LucideIcon }> = [
    ...
    { key: "customers", label: "Clientes", icon: UsersRound }, { key: "payments", label: "Pagos", icon: CreditCard }, { key: "products", label: "Productos", icon: CircleDollarSign }, { key: "reminders", label: "Notificaciones", icon: Bell },
  ];
  if (isAdmin) base.push({ key: "employees", label: "Personal", icon: UserCog }, { key: "operations", label: "Operación", icon: ClipboardList }, { key: "history", label: "Historial", icon: History }, { key: "access", label: "Accesos", icon: ShieldAlert });
...
function AccessPanel({ accessLogs }: { accessLogs: AccessLog[] }) {
  return <><PageTitle eyebrow="Seguridad administrativa" title="Registro de accesos" /><p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">Historial de inicios de sesión registrados por la plataforma. Solo Administración/IT puede consultarlo.</p><section className="panel-card mt-6 overflow-hidden">{accessLogs.length ? <div className="table-wrap"><table><thead><tr><th>Personal</th><th>Rol</th><th>Evento</th><th>Fecha y hora</th></tr></thead><tbody>{accessLogs.map((log) => <tr key={log.id}><td><p className="font-bold">{log.displayName}</p><span>{log.email}</span></td><td><StatusPill status={log.role} /></td><td><span className="rounded-full bg-[#1676F3]/10 px-2.5 py-1 text-xs font-bold text-[#0C58C7]">{log.event === "login" ? "Inicio de sesión" : "Cierre de sesión"}</span></td><td>{readableTimestamp(log.occurredAt)}</td></tr>)}</tbody></table></div> : <Empty title="Sin accesos registrados" detail="Los inicios de sesión posteriores a esta actualización aparecerán aquí." />}</section></>;
}
...
export default function Dashboard({ user, profile }: { user: User; profile: UserProfile }) {
  ...
  useEffect(() => {
    ...
    const stops = [subscribeCollection<Customer>("customers", setCustomers, report), subscribeCollection<Reservation>("reservations", setReservations, report), subscribeCollection<Payment>("payments", setPayments, report), subscribeCollection<Product>("products", setProductOverrides, report), subscribeCollection<GeneralReminder>("generalReminders", setReminders, report)];
    if (isAdmin) stops.push(subscribeCollection<UserProfile>("users", setEmployees, report), subscribeCollection<Invitation>("invitations", setInvitations, report), subscribeCollection<ActivityLog>("activityLogs", setActivityLogs, report), subscribeCollection<AccessLog>("accessLogs", setAccessLogs, report));
  ...
  }, [activityLogs, customers, reservations, payments]);
  ...
  const openDetail = (typeOrEyebrow: RecordType | string, eyebrowOrTitle: string, titleOrData: string | object, dataOrRelated?: object | null, relatedMaybe?: object | null) => {
    const hasExplicitType = ["customer", "reservation", "payment", "product", "employee", "reminder"].includes(typeOrEyebrow);
