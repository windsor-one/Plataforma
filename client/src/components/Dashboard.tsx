/**
 * Heliot iPhone Native: navegación agrupada, listas claras y materiales ligeros.
...
function HeliotMark({ className = "" }: { className?: string }) {
  return <span className={`brand-mark ${className}`} aria-hidden="true"><i /><i /></span>;
}
...
function AppSidebar({ active, onNavigate, isAdmin, collapsed, onToggle }: { active: Section; onNavigate: (section: Section) => void; isAdmin: boolean; collapsed: boolean; onToggle: () => void }) {
  ...
  return <aside className={`sidebar-panel ${collapsed ? "is-collapsed" : ""}`}><div className="flex items-center justify-between gap-2 px-2"><button onClick={() => onNavigate("overview")} title="Ir al Resumen" className={`flex min-w-0 items-center gap-3 text-left ${collapsed ? "justify-center" : ""}`}><HeliotMark className="h-11 w-11" />{!collapsed && <div><span className="text-lg font-bold tracking-tight">Sistema <span className="text-[#0F8F73]">Heliot</span></span><p className="mt-0.5 text-[10px] font-medium uppercase tracking-[.1em] text-muted-foreground">Operaciones</p></div>}</button><button className="icon-button" onClick={onToggle} title={collapsed ? "Expandir menú" : "Colapsar menú"}><Menu size={18} /></button></div><nav className="mt-8 space-y-1">{base.map(({ key, label, icon: Icon }) => <button key={key} title={collapsed ? label : undefined} onClick={() => onNavigate(key)} className={`side-link ${active === key ? "active" : ""} ${collapsed ? "justify-center px-2" : ""}`}><Icon size={18} />{!collapsed && label}</button>)}</nav><div className="mt-auto border-t pt-5">{!collapsed && <p className="px-3 pt-3 text-[10px] font-medium uppercase tracking-[.1em] text-muted-foreground">Con tecnología de Windsor</p>}</div></aside>;
}
  ...
  return <div className="min-h-screen bg-background text-foreground">
    <div className="lg:hidden">
      <div className="sticky top-0 z-30 flex items-center justify-between border-b bg-background/90 px-4 py-3 backdrop-blur">
        <button onClick={() => navigate("overview")} title="Ir al Resumen" className="flex min-w-0 items-center gap-2"><HeliotMark className="h-9 w-9" /><b className="truncate">Sistema Heliot</b></button>
