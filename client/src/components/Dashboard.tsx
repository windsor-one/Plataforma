...
import {
  Bell, BarChart3, CalendarDays, CheckCircle2, ChevronDown, ChevronRight, CircleDollarSign, ClipboardList, CreditCard, FileDown,
  ...
  Leaf, UserCog, UserRound, UsersRound, X, type LucideIcon,
} from "lucide-react";
import { toast } from "sonner";
import { AnimatePresence, motion } from "framer-motion";
import { auth } from "@/lib/firebase";
import {
  bulkDeleteEmployeeProfiles, bulkRemoveRecords, bulkUpdateEmployees, bulkUpdateRecords, createGeneralReminder, createRecord, deleteActivityLog, deleteEmployeeProfile, deleteGeneralReminder, deleteProduct, inviteEmployee, markInternalMessageRead, removeRecord, saveInternalMessage, saveProduct, subscribeCollection, subscribeInternalMessages, updateGeneralReminder,
  recordCarbonUsage, subscribeAttendanceSettings, subscribeCarbonUsage, subscribeEmployeeHrRecords, subscribeHrPolicies, subscribeOwnHrProfile, subscribeSecuritySettings, updateAttendanceSettings, updateCarbonUsageSession, updateEmployee, updateOwnProfile, updateRecord, updateSecuritySettings,
} from "@/lib/firestore";
import type { AccessLog, ActivityLog, AttendanceRecord, AttendanceSettings, CarbonUsage, Customer, EmploymentContract, Expense, GeneralReminder, HrDocument, HrGoal, HrPolicy, HrProfile, Incident, InternalMessage, Invitation, LeaveRequest, LifecycleChecklist, OrganizationUnit, Payment, PaymentKind, PaymentMethod, PaymentStatus, PerformanceReview, PolicyAcknowledgment, Product, ProductCategory, Recognition, Reservation, ReservationStatus, SecuritySettings, Task, TrainingRecord, UserProfile, UserRole, WorkSchedule } from "@/lib/types";
...
type Section = "overview" | "calendar" | "mail" | "reservations" | "customers" | "payments" | "products" | "tasks" | "hr" | "hr_reports" | "performance" | "impact" | "finance" | "employees" | "history" | "operations" | "access" | "pending" | "reminders" | "profile";
const sections: Section[] = ["overview", "calendar", "mail", "reservations", "customers", "payments", "products", "tasks", "hr", "hr_reports", "performance", "impact", "finance", "employees", "history", "operations", "access", "pending", "reminders", "profile"];
...
function PageTitle({ eyebrow, title, actions }: { eyebrow: string; title: string; actions?: ReactNode }) {
  return <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow">{eyebrow}</p><h1 className="mt-1 text-[34px] font-bold leading-[1.08] tracking-[-.04em] sm:text-[40px]">{title}</h1></div>{actions}</div>;
}
...
function AdministrativePendingPanel({ items, onNavigate }: { items: AdministrativePendingItem[]; onNavigate: (section: Section) => void }) {
  ...
  return <section className="admin-pending-panel mt-7" aria-labelledby="admin-pending-title"><header className="flex flex-wrap items-start justify-between gap-4 border-b px-5 py-4"><div><p className="eyebrow">Administración / IT</p><h2 id="admin-pending-title" className="mt-1 text-xl font-bold tracking-[-.025em]">Pendientes de configuración</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">Datos incompletos, asignaciones y revisiones que requieren acción administrativa.</p></div><span className="admin-pending-count">{total} pendiente{total === 1 ? "" : "s"}</span></header>{items.length ? <div className="grid divide-y sm:grid-cols-2 sm:divide-x sm:divide-y-0 xl:grid-cols-3">{items.map(({ id, title, detail, count, section, icon: Icon }) => <button key={id} onClick={() => onNavigate(section)} className="admin-pending-item text-left"><span className="admin-pending-icon"><Icon size={18} /></span><span className="min-w-0 flex-1"><span className="block font-semibold text-foreground">{title}</span><span className="mt-1 block text-sm leading-5 text-muted-foreground">{detail}</span></span><span className="admin-pending-number">{count}</span></button>)}</div> : <div className="flex min-h-28 items-center gap-3 px-5 py-5"><CheckCircle2 className="text-[#1676F3]" size={22} /><div><p className="font-semibold">Configuración al día</p><p className="mt-1 text-sm text-muted-foreground">No se detectaron campos ni asignaciones administrativas pendientes.</p></div></div>}</section>;
}
...
function BulkModeControl({ active, onToggle }: { active: boolean; onToggle: () => void }) {
  return <button className={`secondary-button ${active ? "border-primary bg-primary/10 text-primary" : ""}`} onClick={onToggle}><Pencil size={16} />{active ? "Finalizar edición" : "Editar varios"}</button>;
}
...
function ProductForm({ initial, userId, onDone }: { initial?: Product; userId: string; onDone: () => void }) {
  const [submitting, setSubmitting] = useState(false);
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setSubmitting(true);
...
function AppSidebar({ active, onNavigate, isAdmin, isIT, collapsed, onToggle }: { active: Section; onNavigate: (section: Section) => void; isAdmin: boolean; isIT: boolean; collapsed: boolean; onToggle: () => void }) {
  type NavItem = { key: Section; label: string; icon: LucideIcon };
  type NavGroup = { id: string; label: string; icon: LucideIcon; items: NavItem[] };
  const groups: NavGroup[] = [
    { id: "daily", label: "Operación diaria", icon: ClipboardList, items: [{ key: "calendar", label: "Calendario", icon: CalendarDays }, { key: "reservations", label: "Reservas", icon: CalendarDays }, { key: "customers", label: "Clientes", icon: UsersRound }, { key: "payments", label: "Pagos", icon: CreditCard }, { key: "products", label: "Productos", icon: CircleDollarSign }, { key: "tasks", label: "Tareas e incidencias", icon: ClipboardList }] },
    { id: "team", label: "Equipo", icon: UserCog, items: [{ key: "hr", label: "Recursos Humanos", icon: UserCog }, { key: "reminders", label: "Notificaciones", icon: Bell }] },
    { id: "insights", label: "Paneles y análisis", icon: BarChart3, items: [...(isAdmin ? [{ key: "hr_reports" as Section, label: "Reportes RR. HH.", icon: FileDown }, { key: "performance" as Section, label: "Rendimiento", icon: BarChart3 }, { key: "finance" as Section, label: "Finanzas", icon: CircleDollarSign }] : [])] },
  ];
  if (isAdmin) groups.push({ id: "admin", label: "Administración", icon: ShieldAlert, items: [{ key: "pending", label: "Pendientes", icon: CheckCircle2 }, { key: "operations", label: "Operación", icon: ClipboardList }, { key: "history", label: "Historial", icon: History }] });
  if (isIT) groups.push({ id: "it", label: "Departamento de IT", icon: ShieldAlert, items: [{ key: "employees", label: "Personal y accesos", icon: UserCog }, { key: "access", label: "Seguridad y actividad", icon: ShieldAlert }] });
...
function ReservationForm({ initial, customers, products = [], userId, onDone, linkedCustomer, onOpenCustomerPanel, onCreated }: { initial?: Reservation; customers: Customer[]; products?: Product[]; userId: string; onDone: () => void; linkedCustomer?: Customer | null; onOpenCustomerPanel?: () => void; onCreated?: (reservation: Reservation) => void }) {
  const [submitting, setSubmitting] = useState(false);
...
function PaymentForm({ initial, customers, reservations, payments, products, userId, onDone, linkedCustomer, linkedReservation, onOpenCustomerPanel, onOpenReservationPanel }: { initial?: Payment; customers: Customer[]; reservations: Reservation[]; payments: Payment[]; products: Product[]; userId: string; onDone: () => void; linkedCustomer?: Customer | null; linkedReservation?: Reservation | null; onOpenCustomerPanel?: () => void; onOpenReservationPanel?: (customer: Customer) => void }) {
  ...
  const [amount, setAmount] = useState(String(initial?.amount ?? linkedReservation?.productPrice ?? ""));
  const customerOptions = linkedCustomer && !customers.some((customer) => customer.id === linkedCustomer.id) ? [...customers, linkedCustomer] : customers;
  ...
  const previousPaid = selectedReservation ? payments.filter((payment) => payment.reservationId === selectedReservation.id && payment.status === "paid" && payment.id !== initial?.id).reduce((total, payment) => total + payment.amount, 0) : 0;
  const reservationTotal = selectedReservation?.totalDue || selectedReservation?.productPrice || selectedProduct?.price || Number(amount || 0);
  const pendingBalance = Math.max(0, reservationTotal - previousPaid - Number(amount || 0));
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); setSubmitting(true);
    const form = new FormData(event.currentTarget);
    const customer = customerOptions.find((item) => item.id === form.get("customerId"));
    if (!customer) { toast.error("Selecciona un cliente."); setSubmitting(false); return; }
    const payload = { customerId: customer.id, customerName: customerDisplayName(customer), productId: selectedProduct?.id || selectedReservation?.productId, productName: selectedProduct?.name || selectedReservation?.productName, productCategory: selectedProduct?.category || selectedReservation?.productCategory, productPrice: selectedProduct?.price || selectedReservation?.productPrice, productUnit: selectedProduct?.unit || selectedReservation?.productUnit, reservationId: selectedReservationId || undefined, amount: Number(amount), kind: String(form.get("kind")) as PaymentKind, currency: String(form.get("currency")) as Payment["currency"], method: String(form.get("method")) as PaymentMethod, status: String(form.get("status")) as PaymentStatus, paidAt: String(form.get("paidAt")), notes: String(form.get("notes")).trim() };
    try { initial ? await updateRecord("payments", initial.id, payload, userId) : await createRecord("payments", { ...payload, createdBy: userId }); toast.success(initial ? "Pago actualizado." : "Pago registrado."); onDone(); } catch { toast.error("No fue posible guardar el pago."); } finally { setSubmitting(false); }
  };
  return <form className="form-stack" onSubmit={submit}><label>Cliente<select className="field" required name="customerId" value={selectedCustomerId} onChange={(event) => setSelectedCustomerId(event.target.value)}><option value="" disabled>Selecciona un cliente</option>{customerOptions.map((customer) => <option value={customer.id} key={customer.id}>{customerDisplayName(customer)}</option>)}</select></label>{onOpenCustomerPanel && <button type="button" className="workflow-link" onClick={onOpenCustomerPanel}>+ Registrar cliente en panel lateral</button>}<label>Reserva relacionada <span className="font-normal text-muted-foreground">(recomendado)</span><select className="field" name="reservationId" value={selectedReservationId} onChange={(event) => { const reservation = reservationOptions.find((item) => item.id === event.target.value); setSelectedReservationId(event.target.value); if (reservation) { setSelectedProductId(reservation.productId || ""); setAmount(String(reservation.totalDue || reservation.productPrice || "")); } }}><option value="">Sin asociar</option>{reservationOptions.filter((reservation) => !selectedCustomerId || reservation.customerId === selectedCustomerId).map((reservation) => <option value={reservation.id} key={reservation.id}>{reservation.customerName} · {readableDate(reservation.date)} {reservation.time}</option>)}</select></label>{selectedCustomer && onOpenReservationPanel && <button type="button" className="workflow-link" onClick={() => onOpenReservationPanel(selectedCustomer)}>+ Registrar reserva en panel lateral</button>}<label>Paquete <span className="font-normal text-muted-foreground">(opcional)</span><select className="field" name="productId" value={selectedProductId} onChange={(event) => { const product = products.find((item) => item.id === event.target.value); setSelectedProductId(event.target.value); if (product) setAmount(String(product.price)); }}><option value="">Pago personalizado / sin paquete</option>{products.map((product) => <option value={product.id} key={product.id}>{product.name} · {currency(product.price)} {product.unit}</option>)}</select></label>{selectedReservation && <div className="rounded-xl border border-[#FFC72C]/25 bg-[#FFC72C]/5 px-3 py-2 text-xs leading-5 text-muted-foreground"><strong className="text-foreground">Control de saldo:</strong> total {currency(reservationTotal)} · abonado antes de este pago {currency(previousPaid)} · saldo luego de guardar {currency(pendingBalance)}</div>}<div className="grid gap-4 sm:grid-cols-2"><label>Tipo de pago<select className="field" name="kind" defaultValue={initial?.kind || "full"}><option value="deposit">Anticipo</option><option value="partial">Pago parcial</option><option value="balance">Liquidación de saldo</option><option value="full">Pago completo</option></select></label><label>Importe<input className="field" type="number" name="amount" min="0" step="0.01" required value={amount} onChange={(event) => setAmount(event.target.value)} /></label></div><div className="grid gap-4 sm:grid-cols-2"><label>Moneda<select className="field" name="currency" defaultValue={initial?.currency || "USD"}><option>USD</option><option>EUR</option><option>MXN</option><option>ARS</option><option>COP</option></select></label><label>Método<select className="field" name="method" defaultValue={initial?.method || "card"}><option value="card">Tarjeta</option><option value="cash">Efectivo</option><option value="transfer">Transferencia</option><option value="other">Otro</option></select></label></div><label>Estado<select className="field" name="status" defaultValue={initial?.status || "paid"}><option value="paid">Pagado</option><option value="pending">Pendiente</option><option value="refunded">Reintegrado</option></select></label><label>Fecha<input className="field" type="date" name="paidAt" required defaultValue={initial?.paidAt || dateToday()} /></label><label>Notas<textarea className="field min-h-20" name="notes" defaultValue={initial?.notes} /></label><button className="primary-button" disabled={submitting}>{submitting ? "Guardando…" : "Guardar pago"}<ChevronRight size={16} /></button></form>;
}
...
function EmployeeForm({ initial, adminId, canManageIT, onDone }: { initial?: UserProfile; adminId: string; canManageIT: boolean; onDone: () => void }) {
  ...
  return <form className="form-stack" onSubmit={submit}>
    ...
    <label>Rol<select className="field" name="role" defaultValue={initial?.role || "personal"}><option value="personal">Personal</option><option value="admin">Administrador</option>{canManageIT && <option value="it">Departamento de IT</option>}</select></label>
...
export default function Dashboard({ user, profile }: { user: User; profile: UserProfile }) {
  ...
  const visibleHistory = useMemo<ActivityLog[]>(() => {
    ...
    return registrations.sort((left, right) => readableTimestamp(right.occurredAt).localeCompare(readableTimestamp(left.occurredAt)));
  ...
  const deleteSelectedRecords = async (name: "customers" | "payments", ids: string[], onDone: () => void) => { if (!ids.length || !window.confirm(`¿Eliminar ${ids.length} registro${ids.length === 1 ? "" : "s"} seleccionado${ids.length === 1 ? "" : "s"}? Esta acción no se puede deshacer.`)) return; try { await bulkRemoveRecords(name, ids, user.uid); onDone(); toast.success("Registros eliminados y anotados en Historial."); } catch { toast.error("No se pudieron eliminar todos los registros seleccionados."); } };
  ...
  const updateSelectedPayments = async (status: PaymentStatus) => { if (!selectedPaymentIds.length) return; try { await bulkUpdateRecords("payments", selectedPaymentIds, { status }, user.uid); setSelectedPaymentIds([]); toast.success("Estado actualizado en los pagos seleccionados."); } catch { toast.error("No se pudieron actualizar los pagos seleccionados."); } };
  const updateSelectedEmployees = async (status: UserProfile["status"]) => { const ids = selectedEmployeeIds.filter((id) => id !== user.uid); if (!ids.length) { toast.error("Selecciona perfiles de Personal distintos a tu propia cuenta."); return; } try { await bulkUpdateEmployees(ids, { status }, user.uid); setSelectedEmployeeIds([]); toast.success("Configuración actualizada para el Personal seleccionado."); } catch { toast.error("No se pudo actualizar todo el Personal seleccionado."); } };
  const deleteSelectedEmployees = async () => { const ids = selectedEmployeeIds.filter((id) => id !== user.uid); if (!ids.length || !window.confirm(`¿Eliminar ${ids.length} perfil${ids.length === 1 ? "" : "es"} de Personal? Esta acción no se puede deshacer.`)) return; try { await bulkDeleteEmployeeProfiles(ids, user.uid); setSelectedEmployeeIds([]); toast.success("Perfiles eliminados y anotados en Historial."); } catch { toast.error("No se pudieron eliminar todos los perfiles seleccionados."); } };
  ...
  const navigate = (next: Section) => { impactPageViews.current += 1; impactInteractions.current += 1; if (next !== section) window.history.pushState({ ...window.history.state, sigesSection: next }, "", window.location.href); setSection(next); setBulkEditSection(null); setMobileOpen(false); setQueryText(""); };
  ...
  const modalTitle = modal?.detail?.title || (modal?.type === "customer" ? `${modal.data ? "Editar" : "Nuevo"} cliente` : modal?.type === "reservation" ? `${modal.data ? "Editar" : "Nueva"} reserva` : modal?.type === "payment" ? `${modal.data ? "Editar" : "Registrar"} pago` : modal?.type === "product" ? `${modal.data ? "Editar" : "Nuevo"} paquete` : modal?.type === "reminder" ? "Notificaciones" : modal?.data ? "Configurar personal" : "Invitar personal");
  ...
  return <div className={`min-h-screen bg-background text-foreground ${bulkEditSection === section ? "bulk-editing" : ""}`}>
    <div className="lg:hidden">
      <div className="sticky top-0 z-30 flex items-center justify-between border-b bg-background/90 px-4 py-3 backdrop-blur">
        ...
        <div className="flex shrink-0 items-center gap-2"><button className="icon-button relative" aria-label="Abrir correo interno" onClick={() => navigate("mail")}><Mail size={18} />{unreadMailCount ? <span className="absolute -right-1 -top-1 min-w-4 rounded-full bg-[#C53B53] px-1 text-[10px] font-bold text-white">{unreadMailCount}</span> : null}</button><button className="icon-button" aria-label="Abrir impacto digital" onClick={() => navigate("impact")}><Leaf size={18} /></button><AccountMenu profile={profile} onNavigate={navigate} onLogout={() => void signOut(auth)} /><button className="icon-button" aria-label="Abrir navegación" onClick={() => setMobileOpen(!mobileOpen)}><Menu size={20} /></button></div>
      </div>
      {mobileOpen && <div className="floating-surface border-x-0 border-t-0 p-3"><AppSidebar active={section} onNavigate={navigate} isAdmin={isAdmin} isIT={isIT} collapsed={false} onToggle={() => setMobileOpen(false)} /></div>}
    </div>
    <div className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
      <div className="hidden lg:block"><AppSidebar active={section} onNavigate={navigate} isAdmin={isAdmin} isIT={isIT} collapsed={sidebarCollapsed} onToggle={toggleSidebar} /></div>
      <main className="min-w-0 px-4 pb-28 pt-6 sm:px-7 lg:px-10 lg:py-8">
        <header className="mb-8 flex items-center justify-between gap-4"><div className="hidden text-sm text-muted-foreground sm:block"><span className="font-bold text-foreground">{profile.displayName}</span><span className="mx-2">·</span><StatusPill status={profile.role} /></div><div className="ml-auto flex items-center gap-2"><button onClick={() => setGlobalSearchOpen(true)} title="Buscar en la plataforma" className="secondary-button hidden gap-2 px-3 sm:flex"><Search size={16} />Buscar <kbd className="rounded border bg-muted px-1 py-0.5 text-[10px]">⌘K</kbd></button><button onClick={() => navigate("mail")} title="Correo interno" className="icon-button relative"><Mail size={18} />{unreadMailCount ? <span className="absolute -right-1 -top-1 min-w-4 rounded-full bg-[#C53B53] px-1 text-[10px] font-bold text-white">{unreadMailCount}</span> : null}</button><button onClick={() => navigate("impact")} title="Impacto digital" className="icon-button"><Leaf size={18} /></button><button onClick={toggleTheme} title="Cambiar tema" className="icon-button">{theme === "light" ? <Moon size={18} /> : <Sun size={18} />}</button><AccountMenu profile={profile} onNavigate={navigate} onLogout={() => void signOut(auth)} /></div></header>
        {["customers", "payments", "employees", "hr"].includes(section) && <div className="mb-5 flex justify-end"><BulkModeControl active={bulkEditSection === section} onToggle={() => { const next = bulkEditSection === section ? null : section; if (!next) { setSelectedCustomerIds([]); setSelectedPaymentIds([]); setSelectedEmployeeIds([]); } setBulkEditSection(next); }} /></div>}
        <motion.div key={section} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .22, ease: [0.23, 1, 0.32, 1] }}>
        {section === "overview" ? overview : section === "mail" ? <InternalMailPanel messages={internalMessages} employees={employees} profile={profile} onRead={(message) => void markInternalMessageRead(message.id, user.uid, message.readByIds || []).catch(() => undefined)} /> : section === "customers" ? customersPanel : section === "reservations" ? reservationsPanel : section === "payments" ? paymentsPanel : section === "products" ? productsPanel : section === "tasks" ? <WorkPanel tasks={tasks} incidents={incidents} employees={employees} reservations={reservations} userId={user.uid} isAdmin={isAdmin} /> : section === "hr" ? <HrPanel user={profile} isAdmin={isAdmin} employees={employees} hrProfile={hrProfile} profiles={hrProfiles} units={organizationUnits} contracts={employmentContracts} documents={hrDocuments} schedules={workSchedules} attendance={attendanceRecords} attendanceSettings={attendanceSettings} leaves={leaveRequests} lifecycle={lifecycleChecklists} goals={hrGoals} reviews={performanceReviews} training={trainingRecords} recognitions={recognitions} policies={hrPolicies} acknowledgments={policyAcknowledgments} /> : section === "hr_reports" && isAdmin ? <HrReportsPanel employees={employees} profiles={hrProfiles} attendance={attendanceRecords} leaves={leaveRequests} /> : section === "performance" && isAdmin ? <PerformanceDashboard employees={employees} profiles={hrProfiles} attendance={attendanceRecords} tasks={tasks} goals={hrGoals} reviews={performanceReviews} training={trainingRecords} /> : section === "impact" ? <ImpactPanel usage={carbonUsage} isAdmin={isAdmin} /> : section === "finance" && isAdmin ? <FinancePanel payments={payments} expenses={expenses} reservations={reservations} userId={user.uid} /> : section === "reminders" ? remindersPanel : section === "employees" && isAdmin ? employeesPanel : section === "operations" && isAdmin ? <OperationsPanel reservations={reservations} employees={employees} userId={user.uid} /> : section === "history" && isAdmin ? historyPanel : section === "access" && isAdmin ? <AccessPanel accessLogs={accessLogs} activityLogs={activityLogs} carbonUsage={carbonUsage} /> : profilePanel}
        </motion.div>
      </main>
    </div>
    ...
    <GlobalSearch open={globalSearchOpen} onClose={() => setGlobalSearchOpen(false)} onNavigate={navigate} customers={customers} reservations={reservations} payments={payments} tasks={tasks} incidents={incidents} expenses={expenses} isAdmin={isAdmin} />
  </div>;
}
