...
import type { AccessLog, ActivityLog, AttendanceGuard, AttendanceRecord, AttendanceSettings, CarbonUsage, Customer, EmploymentContract, Expense, GeneralReminder, HrDocument, HrGoal, HrPolicy, HrProfile, Incident, InternalMessage, Invitation, LeaveRequest, LifecycleChecklist, OrganizationUnit, Payment, PaymentKind, PaymentMethod, PaymentStatus, PerformanceReview, PolicyAcknowledgment, Product, ProductCategory, Recognition, Reservation, ReservationStatus, SecuritySettings, Task, TrainingRecord, UpdateRequest, UserProfile, UserRole, WorkSchedule } from "@/lib/types";
...
type Section = "overview" | "calendar" | "mail" | "reservations" | "customers" | "payments" | "products" | "tasks" | "hr" | "updates" | "hr_reports" | "performance" | "impact" | "finance" | "employees" | "history" | "operations" | "access" | "pending" | "reminders" | "profile";
const sections: Section[] = ["overview", "calendar", "mail", "reservations", "customers", "payments", "products", "tasks", "hr", "updates", "hr_reports", "performance", "impact", "finance", "employees", "history", "operations", "access", "pending", "reminders", "profile"];
...
function AppSidebar({ active, onNavigate, isAdmin, isIT, collapsed, onToggle }: { active: Section; onNavigate: (section: Section) => void; isAdmin: boolean; isIT: boolean; collapsed: boolean; onToggle: () => void }) {
  ...
  const groups: NavGroup[] = [
    ...
    { id: "team", label: "Equipo", icon: UserCog, items: [{ key: "hr", label: "Recursos Humanos", icon: UserCog }, { key: "updates", label: "Actualizaciones", icon: ClipboardList }, { key: "reminders", label: "Notificaciones", icon: Bell }] },
