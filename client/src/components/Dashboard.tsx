...
import {
  ...
  Eye, History, LayoutDashboard, LockKeyhole, LogOut, Menu, Moon, PanelLeftClose, PanelLeftOpen, Pencil, Plus, Search, ShieldAlert, Sun, Trash2,
...
function AppSidebar({ active, onNavigate, isAdmin, collapsed, onToggle }: { active: Section; onNavigate: (section: Section) => void; isAdmin: boolean; collapsed: boolean; onToggle: () => void }) {
  ...
  const groups: NavGroup[] = [
    ...
    { id: "insights", label: "Paneles y análisis", icon: BarChart3, items: [{ key: "impact", label: "Impacto digital", icon: Leaf }, ...(isAdmin ? [{ key: "hr_reports" as Section, label: "Reportes RR. HH.", icon: FileDown }, { key: "performance" as Section, label: "Rendimiento", icon: BarChart3 }, { key: "finance" as Section, label: "Finanzas", icon: CircleDollarSign }] : [])] },
  ];
  if (isAdmin) groups.push({ id: "admin", label: "Administración", icon: ShieldAlert, items: [{ key: "employees", label: "Personal", icon: UserCog }, { key: "operations", label: "Operación", icon: ClipboardList }, { key: "history", label: "Historial", icon: History }, { key: "access", label: "Accesos y actividad", icon: ShieldAlert }] });
...
export default function Dashboard({ user, profile }: { user: User; profile: UserProfile }) {
  ...
  return <div className="min-h-screen bg-background text-foreground">
    <div className="lg:hidden">
      ...
      {mobileOpen && <div className="floating-surface border-x-0 border-t-0 p-3"><AppSidebar active={section} onNavigate={navigate} isAdmin={isAdmin} collapsed={false} onToggle={() => setMobileOpen(false)} /></div>}
    </div>
    <div className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
      ...
      <main className="min-w-0 px-4 pb-28 pt-6 sm:px-7 lg:px-10 lg:py-8">
        ...
        {isAdmin && section === "overview" && <AdministrativePendingPanel items={administrativePendingItems} onNavigate={navigate} />}
