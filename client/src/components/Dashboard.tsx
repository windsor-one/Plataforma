...
function AppSidebar({ active, onNavigate, isAdmin, onLogout }: { active: Section; onNavigate: (section: Section) => void; isAdmin: boolean; onLogout: () => void }) {
  ...
  return <aside className="sidebar-panel"><div className="flex items-center gap-3 px-2"><img src="/manus-storage/gestionpro-mark_51952039.png" alt="Heliot Media" className="h-11 w-11 rounded-xl bg-white p-1" /><div><span className="text-lg font-extrabold tracking-tight">Heliot <span className="text-[#0F8F73]">Media</span></span><p className="mt-0.5 text-[10px] font-bold uppercase tracking-[.13em] text-muted-foreground">Operaciones</p></div></div><nav className="mt-10 space-y-1">{base.map(({ key, label, icon: Icon }) => <button key={key} onClick={() => onNavigate(key)} className={`side-link ${active === key ? "active" : ""}`}><Icon size={18} />{label}</button>)}</nav><div className="mt-auto space-y-1 border-t pt-5"><button onClick={() => onNavigate("profile")} className={`side-link ${active === "profile" ? "active" : ""}`}><UserRound size={18} />Mi perfil</button><button onClick={onLogout} className="side-link text-destructive"><LogOut size={18} />Cerrar sesión</button><p className="px-3 pt-3 text-[10px] font-semibold uppercase tracking-[.12em] text-muted-foreground">Con tecnología de Windsor</p></div></aside>;
}
...
function ReservationForm({ initial, customers, userId, onDone }: { initial?: Reservation; customers: Customer[]; userId: string; onDone: () => void }) {
  ...
  const customerOptions = newCustomer ? [...customers, newCustomer] : customers;
...
function PaymentForm({ initial, customers, reservations, userId, onDone }: { initial?: Payment; customers: Customer[]; reservations: Reservation[]; userId: string; onDone: () => void }) {
  ...
  const [selectedCustomerId, setSelectedCustomerId] = useState(initial?.customerId || "");
  const customerOptions = newCustomer ? [...customers, newCustomer] : customers;
  const reservationOptions = newReservation ? [...reservations, newReservation] : reservations;
  ...
  return <form className="form-stack" onSubmit={submit}><label>Cliente<select className="field" required name="customerId" value={selectedCustomerId} onChange={(event) => setSelectedCustomerId(event.target.value)}><option value="" disabled>Selecciona un cliente</option>{customerOptions.map((customer) => <option value={customer.id} key={customer.id}>{customer.fullName}</option>)}</select></label><button type="button" className="text-left text-sm font-bold text-[#08745D] hover:underline dark:text-[#5DDBC0]" onClick={() => setQuickCustomerOpen((open) => !open)}>{quickCustomerOpen ? "Cerrar alta rápida" : "+ Añadir nuevo cliente"}</button>{quickCustomerOpen && <InlineCustomer userId={userId} onCreated={(customer) => { setNewCustomer(customer); setSelectedCustomerId(customer.id); setQuickCustomerOpen(false); }} />}<label>Reserva relacionada <span className="font-normal text-muted-foreground">(opcional)</span><select key={newReservation?.id || initial?.reservationId || "empty"} className="field" name="reservationId" defaultValue={newReservation?.id || initial?.reservationId || ""}><option value="">Sin asociar</option>{reservationOptions.filter((reservation) => !selectedCustomerId || reservation.customerId === selectedCustomerId).map((reservation) => <option value={reservation.id} key={reservation.id}>{reservation.customerName} · {readableDate(reservation.date)} {reservation.time}</option>)}</select></label>{selectedCustomer && <><button type="button" className="text-left text-sm font-bold text-[#08745D] hover:underline dark:text-[#5DDBC0]" onClick={() => setQuickReservationOpen((open) => !open)}>{quickReservationOpen ? "Cerrar alta rápida" : "+ Añadir nueva reserva para este cliente"}</button>{quickReservationOpen && <InlineReservation customer={selectedCustomer} userId={userId} onCreated={(reservation) => { setNewReservation(reservation); setQuickReservationOpen(false); }} />}</>}<div className="grid gap-4 sm:grid-cols-2"><label>Importe<input className="field" type="number" name="amount" min="0" step="0.01" required defaultValue={initial?.amount} /></label><label>Moneda<select className="field" name="currency" defaultValue={initial?.currency || "USD"}><option>USD</option><option>EUR</option><option>MXN</option><option>ARS</option><option>COP</option></select></label></div><div className="grid gap-4 sm:grid-cols-2"><label>Método<select className="field" name="method" defaultValue={initial?.method || "card"}><option value="card">Tarjeta</option><option value="cash">Efectivo</option><option value="transfer">Transferencia</option><option value="other">Otro</option></select></label><label>Estado<select className="field" name="status" defaultValue={initial?.status || "paid"}><option value="paid">Pagado</option><option value="pending">Pendiente</option><option value="refunded">Reintegrado</option></select></label></div><label>Fecha<input className="field" type="date" name="paidAt" required defaultValue={initial?.paidAt || dateToday()} /></label><label>Notas<textarea className="field min-h-20" name="notes" defaultValue={initial?.notes} /></label><button className="primary-button" disabled={submitting}>{submitting ? "Guardando…" : "Guardar pago"}<ChevronRight size={16} /></button></form>;
}
