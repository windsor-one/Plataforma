...
function AppSidebar({ active, onNavigate, isAdmin, collapsed, onToggle }: { active: Section; onNavigate: (section: Section) => void; isAdmin: boolean; collapsed: boolean; onToggle: () => void }) {
  ...
  const [openGroups, setOpenGroups] = useState<Record<string, boolean>>({ daily: true, team: false, insights: false, admin: false });
  useEffect(() => { const currentGroup = groups.find((group) => group.items.some((item) => item.key === active)); if (currentGroup) setOpenGroups((current) => current[currentGroup.id] ? current : { ...current, [currentGroup.id]: true }); }, [active, isAdmin]);
  const toggleGroup = (id: string) => setOpenGroups((current) => ({ ...current, [id]: !current[id] }));
  return <aside className={`sidebar-panel ${collapsed ? "is-collapsed" : ""}`}><div className={`flex flex-col gap-3 px-2 ${collapsed ? "items-center" : "items-stretch"}`}><button onClick={() => onNavigate("overview")} title="Ir al Resumen" className={`flex min-w-0 text-left ${collapsed ? "justify-center" : "flex-col items-start gap-1"}`}><SigesWordmark className={collapsed ? "!text-xl" : "siges-sidebar-wordmark"} />{!collapsed && <p className="text-[10px] font-medium uppercase tracking-[.1em] text-muted-foreground">Sistema Integral de Gestión Estratégica</p>}</button><button className="icon-button sidebar-collapse-control self-end" onClick={onToggle} title={collapsed ? "Expandir menú" : "Colapsar menú"}><Menu size={18} /></button></div><nav className="mt-8"><button title={collapsed ? "Resumen" : undefined} onClick={() => onNavigate("overview")} className={`side-link mb-2 ${active === "overview" ? "active" : ""} ${collapsed ? "justify-center px-2" : ""}`}><LayoutDashboard size={18} />{!collapsed && "Resumen"}</button>{groups.map((group) => { const isGroupActive = group.items.some((item) => item.key === active); const isOpen = Boolean(openGroups[group.id]); const GroupIcon = group.icon; return <section className="sidebar-group" key={group.id}><button aria-expanded={isOpen} onClick={() => collapsed ? onToggle() : toggleGroup(group.id)} className={`sidebar-group-trigger ${isGroupActive ? "has-active" : ""} ${collapsed ? "justify-center" : ""}`} title={collapsed ? group.label : undefined}><GroupIcon size={18} />{!collapsed && <><span>{group.label}</span><ChevronDown className={`ml-auto transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`} size={16} /></>}</button>{!collapsed && isOpen && <div className="sidebar-submenu">{group.items.map(({ key, label, icon: Icon }) => <button key={key} onClick={() => onNavigate(key)} className={`side-sub-link ${active === key ? "active" : ""}`}><Icon size={16} /><span>{label}</span></button>)}</div>}</section>; })}</nav><div className="mt-auto border-t pt-5">{!collapsed && <p className="px-3 pt-3 text-[10px] font-medium uppercase tracking-[.1em] text-muted-foreground">Con tecnología de Windsor</p>}</div></aside>;
}
