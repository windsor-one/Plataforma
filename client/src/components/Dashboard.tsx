...
import {
  ...
  Leaf, UserCog, UserRound, UsersRound, WalletCards, Wifi, WifiOff, X, type LucideIcon,
} from "lucide-react";
...
function AppSidebar({ active, onNavigate, isAdmin, isIT, temporarySections, collapsed, onToggle }: { active: Section; onNavigate: (section: Section) => void; isAdmin: boolean; isIT: boolean; temporarySections: Section[]; collapsed: boolean; onToggle: () => void }) {
  ...
  const temporaryLabels: Partial<Record<Section, string>> = { finance: "Finanzas (temporal)", hr_reports: "Reportes RR. HH. (temporal)", performance: "Rendimiento (temporal)", payroll: "Planilla (temporal)", automations: "Automatizaciones (temporal)", operations: "Operación (temporal)", history: "Historial (temporal)", employees: "Personal (temporal)" };
...
export default function Dashboard({ user, profile }: { user: User; profile: UserProfile }) {
  ...
  const canManagePayroll = isAdmin || hasTemporaryModule("payroll");
  const temporarySections = useMemo<Section[]>(() => Array.from(new Set(temporaryPermissions.flatMap((permission) => permission.module === "finance" ? ["finance" as Section] : permission.module === "hr_reports" ? ["hr_reports" as Section] : permission.module === "performance" ? ["performance" as Section] : permission.module === "payroll" ? ["payroll" as Section] : permission.module === "automations" ? ["automations" as Section] : permission.module === "operations" ? ["operations" as Section] : permission.module === "history" ? ["history" as Section] : permission.module === "employees" ? ["employees" as Section] : []))), [temporaryPermissions]);
  ...
  }, [canManageFinance, canManagePayroll, isAdmin, isIT, user.uid]);
  ...
  return <div className={`min-h-screen bg-background text-foreground ${bulkEditSection === section ? "bulk-editing" : ""}`}>
    ...
    <div className="lg:hidden">
      <div className="sticky top-0 z-30 flex items-center justify-between border-b bg-background/90 px-4 py-3 backdrop-blur">
        ...
        <div className="mobile-header-actions"><button className="icon-button" aria-label="Abrir notificaciones" onClick={() => navigate("reminders")}><Bell size={18} /></button><button className="icon-button relative" aria-label="Abrir correo interno" onClick={() => navigate("mail")}><Mail size={18} />{unreadMailCount ? <span className="absolute -right-1 -top-1 min-w-4 rounded-full bg-[#C53B53] px-1 text-[10px] font-bold text-white">{unreadMailCount}</span> : null}</button><button className="icon-button hidden sm:grid" aria-label="Abrir calendario" onClick={() => navigate("calendar")}><CalendarDays size={18} /></button><button className="icon-button hidden sm:grid" aria-label="Abrir impacto digital" onClick={() => navigate("impact")}><Leaf size={18} /></button><ConnectionStatus isOnline={isOnline} /><button onClick={toggleTheme} title="Cambiar tema" aria-label="Cambiar tema" className="icon-button hidden sm:grid">{theme === "light" ? <Moon size={18} /> : <Sun size={18} />}</button><AccountMenu profile={profile} onNavigate={navigate} onLogout={logout} /><button className="icon-button" aria-label="Abrir navegación" onClick={() => setMobileOpen(!mobileOpen)}><Menu size={20} /></button></div>
      </div>
    ...
    <div className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
      ...
      <main className="min-w-0 px-4 pb-28 pt-6 sm:px-7 lg:px-10 lg:py-8">
        <header className="mb-8 flex items-center justify-between gap-4"><div className="hidden text-sm text-muted-foreground sm:block"><span className="font-bold text-foreground">{profile.displayName}</span><span className="mx-2">·</span><StatusPill status={profile.role} /></div><div className="desktop-header-actions"><button onClick={() => setGlobalSearchOpen(true)} title="Buscar en la plataforma" className="secondary-button hidden gap-2 px-3 sm:flex"><Search size={16} />Buscar <kbd className="rounded border bg-muted px-1 py-0.5 text-[10px]">⌘K</kbd></button><button onClick={() => navigate("reminders")} title="Notificaciones" className="icon-button"><Bell size={18} /></button><button onClick={() => navigate("mail")} title="Correo interno" className="icon-button relative"><Mail size={18} />{unreadMailCount ? <span className="absolute -right-1 -top-1 min-w-4 rounded-full bg-[#C53B53] px-1 text-[10px] font-bold text-white">{unreadMailCount}</span> : null}</button><button onClick={() => navigate("calendar")} title="Calendario" className="icon-button"><CalendarDays size={18} /></button><button onClick={() => navigate("impact")} title="Impacto digital" className="icon-button"><Leaf size={18} /></button><ConnectionStatus isOnline={isOnline} /><button onClick={toggleTheme} title="Cambiar tema" aria-label="Cambiar tema" className="icon-button">{theme === "light" ? <Moon size={18} /> : <Sun size={18} />}</button><AccountMenu profile={profile} onNavigate={navigate} onLogout={logout} /></div></header>
