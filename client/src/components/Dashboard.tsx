...
function AppSidebar({ active, onNavigate, isAdmin, collapsed, onToggle }: { active: Section; onNavigate: (section: Section) => void; isAdmin: boolean; collapsed: boolean; onToggle: () => void }) {
  ...
  return <aside className={`sidebar-panel ${collapsed ? "is-collapsed" : ""}`}><div className="flex items-center justify-between gap-2 px-2"><button onClick={() => onNavigate("overview")} title="Ir al Resumen" className={`flex min-w-0 items-center gap-3 text-left ${collapsed ? "justify-center" : ""}`}><SigesWordmark className={collapsed ? "text-base" : ""} />{!collapsed && <p className="text-[10px] font-medium uppercase tracking-[.1em] text-muted-foreground">Sistema Integral de Gestión Estratégica</p>}</button><button className="icon-button" onClick={onToggle} title={collapsed ? "Expandir menú" : "Colapsar menú"}><Menu size={18} /></button></div><nav className="mt-8 space-y-1">{base.map(({ key, label, icon: Icon }) => <button key={key} title={collapsed ? label : undefined} onClick={() => onNavigate(key)} className={`side-link ${active === key ? "active" : ""} ${collapsed ? "justify-center px-2" : ""}`}><Icon size={18} />{!collapsed && label}</button>)}</nav><div className="mt-auto border-t pt-5">{!collapsed && <p className="px-3 pt-3 text-[10px] font-medium uppercase tracking-[.1em] text-muted-foreground">Con tecnología de Windsor</p>}</div></aside>;
}
