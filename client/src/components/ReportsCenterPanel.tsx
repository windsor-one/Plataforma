...
import { buildFinanceSummary, inFinancePeriod } from "@/lib/financeReports";
...
function genericPdf(report: Report, period: string, payments: Payment[], expenses: Expense[], reservations: Reservation[]) {
  ...
  const periodPayments = payments.filter(item => inFinancePeriod(item.paidAt, period) && item.status === "paid");
  const periodExpenses = expenses.filter(item => !item.archived && inFinancePeriod(item.spentAt, period));
  const rows: string[][] = [];
