...
Diagnóstico del inicio de sesión: GestionPro solo acepta cuentas creadas en Firebase Authentication y, después de autenticar, exige un perfil activo en `users/{uid}` o una invitación pendiente para ese correo. El proyecto no tenía un administrador creado en la aplicación, y Cloud Firestore no pudo verificarse ni inicializarse desde el navegador disponible; por tanto, una cuenta de Google o un correo sin usuario de Firebase y perfil administrativo no puede entrar.

Ejecución directa: se creó la base `(default)` de Cloud Firestore en edición Standard, modo producción y ubicación `nam5`. La política de roles, invitaciones y empleados activos fue cargada en el editor de Firestore; la consola no expuso todavía el control de publicación, por lo que la regla activa sigue siendo la política inicial de denegación total y debe verificarse antes de probar un inicio de sesión.

Validación pública: el sitio de GitHub Pages carga la pantalla de empleados con la configuración Firebase embebida. Un intento controlado con credenciales inexistentes llegó a Firebase Authentication y devolvió el mensaje de credenciales incorrectas, confirmando la conexión de autenticación. La validación con una cuenta real permanece bloqueada únicamente por la publicación pendiente de reglas y por la sesión de propietario de Firebase.
